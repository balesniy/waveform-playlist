...
class Kali {
    constructor(channels) {
        const t = new Tempo(new TypedQueue(Float32Array), new TypedQueue(Float32Array));
        t.channels = channels;
        this.t = t;
    }
    ...
    static tempoBestOverlapPosition(t, newWin) {
        ...
        let diff;
        let leastDiff = Kali.difference(newWin.subarray(t.channels * i), f, t.channels * t.overlap);
        let k = 0;
            ...
            do {
                ...
                for (k = -1; k <= 1; k += 2) {
                    for (j = 1; j < 4 || step == 64; j++) {
                        ...
                        diff = Kali.difference(newWin.subarray(t.channels * i), f, t.channels * t.overlap);
                        if (diff < leastDiff) {
                            leastDiff = diff;
                            bestPos = i;
                        }
    ...
    process() {
        ...
        while (t.inputFifo.occupancy() >= t.processSize) {
            ...
            if (!t.segmentsTotal) {
                offset = t.search / 2;
                t.outputFifo.write(t.inputFifo.readPtr(t.channels * offset, t.overlap), t.overlap);
            }
            else {
                ...
                Kali.tempoOverlap(t, t.overlapBuf, t.inputFifo.readPtr(t.channels * offset), t.outputFifo.writePtr(t.overlap));
            }
            /* Copy the middle bit to the output */
            t.outputFifo.write(t.inputFifo.readPtr(t.channels * (offset + t.overlap)), t.segment - 2 * t.overlap);
    ...
    setup(sampleRate, factor, // Factor to change tempo by
    quickSearch = false, segmentMs = null, searchMs = null, overlapMs = null) {
        const profile = 1;
        this.reset();
        const t = this.t;
        t.sampleRate = sampleRate;
        if (segmentMs == null) {
            segmentMs = Math.max(10, Kali.segmentsMs[profile] /
        ...
        t.overlap = Math.max(handleInt((sampleRate * overlapMs) / 1000 + 0.5), 16);
